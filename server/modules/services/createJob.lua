...
local function notify(notifyType, resourceName, message, ...)
  ...
  if not NOTIFY_TYPES[notifyType] then
    return lib.print.info(NOTIFY_TYPES.INFO:format(resourceName, formattedMessage))
  end

  return lib.print.info(NOTIFY_TYPES[notifyType]:format(resourceName, formattedMessage))
end
